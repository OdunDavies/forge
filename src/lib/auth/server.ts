...
export const auth = betterAuth({
  ...
  secret: env("BETTER_AUTH_SECRET") ?? stableAuthSecret(),
  database,
  ...
  session: { cookieCache: { enabled: true, maxAge: 300 }, deferSessionRefresh: true },

  // Local email/password — toggled only via `./email-password` (not a plugin).
  ...(emailAndPasswordEnabled ? { emailAndPassword: { enabled: true } } : {}),

  // Native Google for Vercel. The Grok broker preview client only allows
  // `*.grok-sandbox.com` callbacks, so production Google must use Google Cloud OAuth.
